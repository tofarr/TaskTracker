class Attachment < ApplicationRecord
  ...
  def self.viewable_attachments(user)
    ...
    Attachment.joins(:task).where("viewable=true or created_user_id=? or assigned_user_id=?", user.id, user.id)
  end

  def self.editable_attachments(user)
    return Attachment.all if user.admin?
    Attachment.joins(:task).where("editable=true or created_user_id=? or assigned_user_id=?", user.id, user.id)
  end
